  ...
  <div class="collections">
    ...
    <section class="content-warp">
      <div class="content">
        ...
        <div class="page">
          <ul class="page-list">
            <li v-for="i in pageNum" :class="{ cur: myPageCur==i}" @click="changePage(i)">{{i}}</li>
            <li @click="changePage(myPageCur+1)">next</li>
          </ul>
        </div>
      </div>
    </section>
...
export default {
  ...
  computed: {
    ...
    myPageCur () {
      return this.$store.state.myPageCur
    },
    name () {
  ...
  created:function () {
    this.$store.dispatch('produce',{page:1,num:2})
  },
  methods:{
    changePage:function (str) {
      ...
      //获取产品列表
      this.$store.dispatch('produce',{page:str,num:2})
    }
  }
}
</script>
